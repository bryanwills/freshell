...
/// Arm the opencode session locator for a freshly-created REST terminal, iff
/// it's a fresh (non-resuming) opencode pane with a resolved cwd -- mirrors
/// `crates/freshell-ws/src/opencode_association::maybe_arm` EXACTLY (same
/// shared-instance `arm()` call, same argument shape); that wrapper fn is
/// `pub(crate)` inside `freshell-ws` and unreachable from this crate
/// (circular-dependency boundary, see this module's top doc), so this is the
/// thin, crate-local equivalent -- the actual mode/resume/cwd admission logic
/// lives ONCE, inside `OpencodeLocator::arm` itself (shared by both crates
/// via `freshell-sessions`), not duplicated here.
...
pub(crate) fn maybe_send_keys(
    ...
    // `crates/freshell-ws/src/opencode_association.rs`): a REST-created fresh
    // opencode pane only associates once its FIRST submit-shaped input (a
    // bare CR/LF run) is observed here -- a REST `send-keys` must feed the
    // SAME shared locator the WS `terminal.input` path does, or a REST-driven
    // Enter would silently never open the locator's correlation window.
    // No-ops (`note_submit` itself checks "is this terminal armed?") for
    // every non-armed/non-Enter case.
...
/// `crates/freshell-ws/src/opencode_association.rs`): the input is
