...
fn client_types_match_inventory_exactly() {
    ...
    assert_eq!(
        ...
        Some(30),
        "inventory declares 30 client→server types"
    );
    ...
    assert_eq!(actual.len(), 30, "crate declares 30 client types (no dups)");
    assert_eq!(
        actual, expected,
...
#[test]
fn combined_surface_is_90() {
    let all = all_message_types();
    assert_eq!(all.len(), 90, "30 client + 60 server = 90 discriminants");
    // sorted + unique
